...
class PlayState extends FlxNapeState
{
	...
	{
		if (player.y + player.height * 0.5 < FlxG.camera.bounds.y + FlxG.camera.bounds.height * 0.5) {
			FlxG.camera.bounds.y = player.y + player.height * 0.5 - FlxG.camera.bounds.height * 0.5;
		}
		floorBody.position.y = FlxG.camera.bounds.y;
		
		var boundsY = Math.min(0, player.y -500);
		
		for (platform in platforms) {
			if (platform.y > FlxG.camera.bounds.y + FlxG.camera.bounds.height) {
				var newX = FlxG.width * Math.random();
				var newY = FlxG.camera.bounds.y - platform.height * 0.5;
				platform.body.position.setxy(newX, newY);
			}
			
		}
		//FlxG.camera.setBounds(0, boundsY, FlxG.width, FlxG.height);
